...
public class controlRole {
	...
	public ModelAndView ajout(@ModelAttribute("role")Role role) {
		...
		User u=serviceAdmin.getById(role.getUser().getIdUser());
		role.setUser(u);
	...
	@RequestMapping("/user/redirectAccueil")
	public ModelAndView redirectUser() {
		return new ModelAndView("AccueilUser");
	}
	
	@RequestMapping("/admin/redirectAccueil")
	public ModelAndView redirectAdmin() {
		return new ModelAndView("GestionCollege");
	}
}
